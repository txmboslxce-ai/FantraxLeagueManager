...
class ManagerMonth(db.Model):
    ...
    def has_fixtures(self):
        ...
        # Find the latest gameweek with completed fixtures in this season
        latest_completed_gw = Gameweek.query.join(
        ...
        latest_completed_number = latest_completed_gw.number if latest_completed_gw else 0
        
        for gameweek in gameweeks:
            # If this gameweek is beyond what has been completed in the season, 
            # the month is not complete yet
            if gameweek.number > latest_completed_number:
                return False
                
            # Count total fixtures for this gameweek
            total_fixtures = Fixture.query.filter(
                Fixture.gameweek_id == gameweek.id
            ).count()
            
            # If no fixtures exist for this gameweek, but the season has progressed beyond it,
            ...
            # Check if all fixtures for this gameweek have scores
            if fixtures_with_scores < total_fixtures:
                return False
    ...
    def get_standings(self, division_id=None):
        """Get team standings for this month, optionally filtered by division."""
    ...
    def get_division_winner(self, division_id):
        """Get the winner for a specific division in this month."""
        standings = self.get_standings(division_id)
        if standings:
            return standings[0]  # Top team in the standings
        return None

    def get_all_division_winners(self):
        """Get winners for all divisions that have teams in this month."""
        ...
        for division in divisions:
            winner = self.get_division_winner(division.id)
            if winner:
                winners.append({
                    'division': division,
                    'winner': winner
...
class ManagerOfTheMonth(db.Model):
    ...
    division = db.relationship('Division', backref='motm_awards')
