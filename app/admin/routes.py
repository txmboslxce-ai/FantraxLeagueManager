    ...
    if current_season:
        ...
        if form.validate_on_submit():
            ...
            if errors:
                ...
                # Update Manager of the Month winners for each division
                    ...
                    if month.has_fixtures:
                        ...
                        for division in divisions:
                            ...
                            if standings:
                                ...
                                # Check if award exists for this month and division
                                ...
                                if existing_award:
                                    if existing_award.team_id != winner.id:  # Only update if winner changed
                                        existing_award.team_id = winner.id
                                        existing_award.total_score = standings[0]['goals_for']
                                        flash(f'{winner.name} has won {month.name} ({division.name})!', 'success')
                                else:
                        ...
                        # Set the overall winner for backward compatibility (use Premier League winner if available)
                        premier_league = next((d for d in divisions if d.name == 'Premier League'), None)
                        if premier_league:
                            premier_standings = month.get_standings(premier_league.id)
                            if premier_standings:
                                month.winner_id = premier_standings[0]['team'].id
                        elif divisions:
                            # Fallback to first division if no Premier League
                            first_standings = month.get_standings(divisions[0].id)
                            if first_standings:
                                month.winner_id = first_standings[0]['team'].id
...
def manage_cups():
    ...
    current_season = Season.query.filter_by(is_current=True).first()
    if current_season and form.validate_on_submit():
        cup = CupCompetition(
            name=form.name.data, 
            season_id=current_season.id,
...
def manage_manager_month():
    ...
    if current_season:
        ...
        if form.validate_on_submit():
            ...
            # Calculate standings and set winners for each division
            if month.has_fixtures:
                # Get all divisions that have teams playing in this month
                ...
                for division in divisions:
                    ...
                    if standings:
                        ...
                        db.session.add(award)
                        winners_created.append(f'{winner.name} ({division.name})')
                
                if winners_created:
                    # Set the overall winner to the first one (for backward compatibility)
                    if divisions:
                        first_division_standings = month.get_standings(divisions[0].id)
                        if first_division_standings:
                            month.winner_id = first_division_standings[0]['team'].id
                    
                    flash(f'Manager of the Month winners: {", ".join(winners_created)}!', 'success')
                else:
